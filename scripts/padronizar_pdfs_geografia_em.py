...
from core.referencias_geografia import titulos_referencia_geografia_por_docx


BASE_EM_3B = Path(r"D:\PDF novos\GEOGRAFIA\EM\3_BIMESTRE")
...
def _localizar_docx_referencia(pasta: Path) -> Path | None:
    candidatos = sorted(
        [c for c in pasta.glob("Metodologias_Geografia*.docx") if not c.name.startswith("~$")],
...
def padronizar_pasta(pasta: Path) -> dict[str, int]:
    pasta = pasta.resolve()
    base_resolvida = BASE_EM_3B.resolve()
    ...
    titulos = titulos_referencia_geografia_por_docx(docx)
    if not titulos:
...
def main() -> None:
    ...
    print("Padronizacao de Geografia EM - 3o bimestre")
    for pasta in pastas:
