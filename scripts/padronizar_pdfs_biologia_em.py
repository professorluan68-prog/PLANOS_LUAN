...
from core.referencias_biologia import titulos_referencia_biologia_por_docx


BASE_EM_3B = Path(r"D:\PDF novos\BIOLOGIA\EM\3_BIMESTRE")
...
def _localizar_docx_referencia(pasta: Path) -> Path | None:
    candidatos = sorted(
        [c for c in pasta.glob("Metodologias_Biologia*.docx") if not c.name.startswith("~$")],
...
def padronizar_pasta(pasta: Path) -> dict[str, int]:
    ...
    titulos = titulos_referencia_biologia_por_docx(docx)
    if not titulos:
...
def main() -> None:
    ...
    print("Padronizacao de Biologia EM - 3o bimestre")
    for pasta in pastas:
