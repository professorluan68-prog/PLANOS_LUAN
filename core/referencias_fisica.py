"""Referencias prontas de Fisica a partir de DOCX na pasta dos PDFs."""
...
def localizar_docx_referencia_fisica(caminho_pdf: str | Path) -> Path | None:
    caminho = Path(caminho_pdf)
    if not caminho_pdf or not caminho.parent.exists():
        return None

    candidatos = list(caminho.parent.glob("Metodologias_Fisica*.docx"))
    candidatos.extend(caminho.parent.glob("*Fisica*Metodologia*.docx"))
    ...
    candidatos_validos = [
        candidato for candidato in candidatos_unicos if not candidato.name.startswith("~$")
    ]
...
def titulos_referencia_fisica_por_docx(caminho_docx: str | Path) -> dict[int, str]:
...
def referencia_fisica_por_pdf(caminho_pdf: str | Path, numero_aula: Any, tema: str = "") -> dict[str, Any] | None:
    docx = localizar_docx_referencia_fisica(caminho_pdf)
    if not docx:
        return None
