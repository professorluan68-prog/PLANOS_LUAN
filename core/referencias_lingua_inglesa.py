"""Referencias prontas de Lingua Inglesa a partir de DOCX na pasta dos PDFs."""
...
def _carregar_referencias_docx(caminho_docx: str) -> dict[int, dict[str, Any]]:
    return carregar_referencias_docx(caminho_docx,
        ...
        capturar_habilidade=True,
    )




def localizar_docx_referencia_lingua_inglesa(caminho_pdf: str | Path) -> Path | None:
    caminho = Path(caminho_pdf)
    if not caminho_pdf or not caminho.parent.exists():
        return None

    candidatos = list(caminho.parent.glob("Metodologias_Lingua_Inglesa*.docx"))
    candidatos.extend(caminho.parent.glob("*Lingua*Inglesa*Metodologia*.docx"))
    candidatos.extend(caminho.parent.glob("*Língua*Inglesa*Metodologia*.docx"))
...
def titulos_referencia_lingua_inglesa_por_docx(caminho_docx: str | Path) -> dict[int, str]:
...
def referencia_lingua_inglesa_por_pdf(caminho_pdf: str | Path, numero_aula: Any, tema: str = "") -> dict[str, Any] | None:
    docx = localizar_docx_referencia_lingua_inglesa(caminho_pdf)
    if not docx:
        return None
