"""Referencias prontas de Arte a partir de DOCX na pasta dos PDFs."""
...
def localizar_docx_referencia_arte(caminho_pdf: str | Path) -> Path | None:
    caminho = Path(caminho_pdf)
    if not caminho_pdf or not caminho.parent.exists():
        return None

    candidatos = list(caminho.parent.glob("Metodologias_Arte*.docx"))
    candidatos.extend(caminho.parent.glob("*Arte*Metodologia*.docx"))
...
def titulos_referencia_arte_por_docx(caminho_docx: str | Path) -> dict[int, str]:
...
def referencia_arte_por_pdf(caminho_pdf: str | Path, numero_aula: Any, tema: str = "") -> dict[str, Any] | None:
    docx = localizar_docx_referencia_arte(caminho_pdf)
    if not docx:
        return None
