...
def _limpar_titulo_material(linha: str, disciplina: str) -> str:
    titulo = re.sub(r"\s+", " ", linha or "").strip(f" -{_DASH_CHARS}")
    ...
    if disciplina_norm and titulo_norm.startswith(disciplina_norm):
        titulo = titulo[len(disciplina) :].strip(f" -{_DASH_CHARS}:")
        titulo = re.sub(
    ...
    if _linha_periodo_ensino(titulo):
        return ""
    return titulo.strip(f" -{_DASH_CHARS}")
...
def _titulo_em_linha_aula(linha: str) -> str:
    texto = re.sub(r"\s+", " ", str(linha or "")).strip(f" -:{_DASH_CHARS}")
    match = re.match(
        r"^aula\s*(?:n[.o]?\s*)?\d{1,3}\s*(?:[|:-]|\u2013|\u2014)?\s*(.+)$",
        texto,
        flags=re.I,
    )
    if not match:
        return ""
    titulo = match.group(1).strip(f" -:{_DASH_CHARS}")
    if not titulo:
        return ""
